...
const create = async (
  ...
  userName,
  userEmail,
  accessLevel = taskAccessLevels.view
) => {
  ...
  try {
    ...
    if (rows.length === 0 || rows[0].access_level !== taskAccessLevels.delete)
      throw new ApiError(400, "bad request");
    ...
    if (error?.code === "23514") {
      throw new ApiError(
        400,
        `bad request - invalid access level (valid are '${taskAccessLevels.view}', '${taskAccessLevels.edit}' or '${taskAccessLevels.delete}')`
      );
    }
    throw error;
  }
...
const edit = async (user, taskId, userId, userName, userEmail, accessLevel) => {
  ...
  try {
    ...
    if (rows.length === 0 || rows[0].access_level === taskAccessLevels.view)
      throw new ApiError(400, "bad request");

    if (
      rows[0].access_level === taskAccessLevels.edit &&
      accessLevel === taskAccessLevels.delete
    )
      throw new ApiError(400, "bad request");
